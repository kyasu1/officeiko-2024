    ...
    , body =
        ...
        , article [ class "max-w-[768px] mx-auto" ]
            [ section [ class "px-2 space-y-4 mb-4" ]
                [ div [ class "text-right pt-4 print:pt-0" ] [ text "更新日：", text <| Market.updated app.sharedData.market ]
                , div [ class "text-sm grid grid-cols-[200px_1fr_1fr] w-full md:max-w-[600px] mx-auto" ]
                    ...
                    , div [ class "text-right pr-2" ] [ text (Market.gdPrice app.sharedData.market), text "円（税込み）" ]
                    , div [ class "text-right pr-2" ]
                        [ text "前日比 "
                        , text (Market.gdDiff app.sharedData.market)
                        , text "円"
                        ]
                    , div [ class "text-center" ] [ text "プラチナ" ]
                    , div [ class "text-right pr-2" ] [ text (Market.ptPrice app.sharedData.market), text "円（税込み）" ]
                    , div [ class "text-right pr-2" ]
                        [ text "前日比 "
                        , text (Market.ptDiff app.sharedData.market)
                        , text "円"
                        ]
                    ]
                , viewGd (Market.gdRates app.sharedData.market)
                , div [ class "text-sm print:hidden" ] [ text "`18K/14K`刻印はX線でお調べしたうえでお買取いたします（一部買取不可あり）" ]
                , viewPt (Market.ptRates app.sharedData.market)
                , div [ class "text-sm print:hidden" ] [ text "`Pm/PM`刻印はPt850として買取いたします" ]
                , viewSv (Market.svRates app.sharedData.market)
                , div [ class "text-sm print:hidden" ] [ text "`SILVER`の刻印のみの場合は原則として買取不可となります" ]
                ]
            , section [ class "px-2 space-y-4 mb-4 print:hidden" ]
                [ viewCoin app.sharedData.market
